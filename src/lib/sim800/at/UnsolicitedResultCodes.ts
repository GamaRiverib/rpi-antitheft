...
export class UnsolicitedResultCodes {
    ...
    public static readonly CMGS: UnsolicitedResultCode = { code: 'CMGS', regexp: /\+CMGS:/i };

    public static readonly CMSS: UnsolicitedResultCode = { code: 'CMSS', regexp: /\+CMSS:/i };

    public static readonly SMTPSEND: UnsolicitedResultCode = { code: 'SMTPSEND', regexp: /\+SMTPSEND:/i };

    private static codes: UnsolicitedResultCode[] = [];

    public static evalute(val: string): UnsolicitedResultCode {
        if(UnsolicitedResultCodes.codes.length == 0) {
            for(let k in UnsolicitedResultCodes) {
                if(UnsolicitedResultCodes[k].code && UnsolicitedResultCodes[k].regexp) {
                    UnsolicitedResultCodes.codes.push(UnsolicitedResultCodes[k]);
                }
            }
        }
        let urc: UnsolicitedResultCode;
        let index = -1;
        UnsolicitedResultCodes.codes.forEach((v: UnsolicitedResultCode, i: number) => {
        ...
        if (index >= 0) {
            urc = UnsolicitedResultCodes.codes[index];
        } 
        return urc;
    }
}