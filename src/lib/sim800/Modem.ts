...
import { UnsolicitedResultCode, UnsolicitedResultCodes } from './at/UnsolicitedResultCodes';
...
export class Modem implements IModem {
    ...
    public onData: (info: UrcInfo) => void;
    ...
    private onDataEventHandler(buffer: Buffer): void {
        ...
        if(this.response.match(this.MESSAGE_END)) {
            ...
            if(lines.length > 2 && this.tail.length > 0) {
                ...
                if(!lines[0].endsWith(item.atCommand.command)) {
                    console.log(item);
                    return item.errorCallback('Command not match');
                }
            ...
            if(this.tail.length > 0) {
                ...
                if(cmd.pdu) {
                    this.serialPort.write(cmd.pdu, cmd.encoding || 'ascii', (err: any, bytesWritten: number) => {
                        // console.log(cmd.pdu); // TODO: remove this line
                    });
                }
            }
        } else if(this.response.endsWith(this.LINE_END) && this.tail.length == 0) {
            if(this.onData) {
                let line = this.response.toString();
                let urc: UnsolicitedResultCode = UnsolicitedResultCodes.evalute(line);
                if(urc) {
                    console.log('[urc]', this.response.replace(/\r/g, '<CR>').replace(/\n/g,'<LF>')); // TODO: remove this line
                    let data = line.replace(urc.regexp, '');
                    let info: UrcInfo = { urc: urc, line: line, data: data };
                    this.onData(info);
                }
            }
            this.response = '';
        ...
        if(this.tail.length > 0) {
            ...
            this.serialPort.write(cmd + this.SEND_CHAR, 'ascii', (err: any, bytesWritten: number) => {
                ...
                let timer = setTimeout(() => {
                    console.log('[timeout] ', item.atCommand);
                    let error: ModemTimeout = { event: 'timeout' };
                    item.errorCallback(error);
                }, item.atCommand.timeout || 3000);
                this.tail[0].timer = timer;
            });
        }
    }
