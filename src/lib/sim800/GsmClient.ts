import { IModem, UrcInfo } from "./Modem";
...
export class GenericGsmClient implements IGsmClient {
    ...
    private onDataHandler(info: UrcInfo): void {
        this.eventEmmiter.emit(info.urc.code, info);
        
        if(info.urc.code == 'CUSD') {
            let parts = info.data.split(',');
        ...
        } else if(info.urc.code == 'CMGS') {
            let i = info.line.split(':')[1].replace(/\r\n/g, '').replace(/\s/g, '');
